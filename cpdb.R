...
cpdb_pre <- function(input,dir){
  ## prepare meta data
  metadata <- input@meta.data[,c("barcode","cell_type")]
  colnames(metadata) <- c("Cell","cell_type")
  
  ## prepare count data
  count_raw <- input@assays$RNA@counts
  dim(count_raw)
  if (ncol(count_raw)<60000){
    count_norm <- apply(count_raw, 2, function(x) (x/sum(x)*10000)) ## normalization
  }else{
    print("cell number is more than 6w.")
  }
  write.table(metadata,paste0(dir,"meta.txt"),row.names = FALSE,sep="\t",quote=F)
  write.table(count_norm,paste0(dir,"count.txt",sep='\t',quote=F))
}

## heatmap
cpdb_heatmap <- function(dir){
  library(pheatmap)
  setwd(dir)
  meta_file <- paste0(dir,'/meta.txt')
  mean_file <- paste0(dir,'out/means.txt')
  pvalues_file <- paste0(dir,'out/pvalues.txt')
  ...
  if (any(count1)>0){
    ...
    pheatmap::pheatmap(count_matrix, show_rownames = show_rownames, show_colnames = show_colnames, scale=scale, cluster_cols = cluster_cols,
                       ...
                       main = main, cellwidth = 40, cellheight = 40, treeheight_row = treeheight_row, family = family,color = col.heatmap, treeheight_col = treeheight_col, filename = count_filename)
    ...
    pheatmap::pheatmap(log(count_matrix+1), show_rownames = show_rownames, show_colnames = show_colnames, scale=scale, cluster_cols = cluster_cols,
                       border_color=border_color, cluster_rows = cluster_rows, fontsize_row = fontsize_row, fontsize_col = fontsize_col,
                       main = main, cellwidth = 40, cellheight = 40, treeheight_row = treeheight_row, family = family,color = col.heatmap, treeheight_col = treeheight_col, filename = log_filename)
  } else {
...
cpdb_dotplot <- function(dir){
  ...
  library(Cairo)
  setwd(dir)
  mfile <- read.csv(paste0(dir,"/out/means.txt"),sep="\t",check.names = F)
  ...
  ## p-value prepare
  pfile <- read.csv(paste0(dir,"out/pvalues.txt"),sep='\t',check.names=F)
